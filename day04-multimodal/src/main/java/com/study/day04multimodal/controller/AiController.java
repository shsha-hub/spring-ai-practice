...
public class AiController {
    ...
    public ReceiptInfo imageAnalysis(@RequestParam MultipartFile file, @RequestParam String conversationId) {
        ...
        return multimodalService.analyzeFileToEntity(file, MediaCategory.IMAGE, conversationId, prompt, ReceiptInfo.class);
    }
    ...
    public String imageDescribe(@RequestParam MultipartFile file, @RequestParam String conversationId) {
        ...
        return multimodalService.analyzeFileToString(file, MediaCategory.IMAGE, conversationId, prompt);
    }

    @PostMapping("/pdf-analysis")
    public String pdfAnalysis(@RequestParam MultipartFile file, @RequestParam String conversationId) {
        String prompt = "이 문서의 내용을 요약해주세요.";
        return multimodalService.analyzeFileToString(file, MediaCategory.PDF, conversationId, prompt);
    }

    @PostMapping("/pdf-describe")
    public PdfSummary pdfDescribe(@RequestParam MultipartFile file, @RequestParam String conversationId) {
        String prompt = "입력받은 PDF 파일의 내용을 친절하게 요약해서 설명해주세요.";
        return multimodalService.analyzeFileToEntity(file, MediaCategory.PDF, conversationId, prompt, PdfSummary.class);
    }
    ...
    public String audioDescribe(@RequestParam MultipartFile file, @RequestParam String conversationId) {
        ...
        return multimodalService.analyzeFileToString(file, MediaCategory.AUDIO, conversationId, prompt);
    }
    ...
    public MeetingBoard analyzeBoard(@RequestParam MultipartFile file, @RequestParam String conversationId) {
        String prompt = "이 화이트보드 사진 속 주제들과 액션 아이템을 정리해주세요.";
        return multimodalService.analyzeFileToEntity(file, MediaCategory.IMAGE, conversationId, prompt, MeetingBoard.class);
    }
    ...
    public AgendaSummary analyzeAgenda(@RequestParam MultipartFile file, @RequestParam String conversationId) {
        ...
        return multimodalService.analyzeFileToEntity(file, MediaCategory.PDF, conversationId, prompt, AgendaSummary.class);
    }

    @PostMapping("/meeting/audio")
    public String analyzeMeetingAudio(@RequestParam MultipartFile file, @RequestParam String conversationId) {
